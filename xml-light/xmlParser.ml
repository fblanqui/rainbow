...
let pos s = Xml_lexer.pos s.source

let rec read_node s =
	match pop s with
	| Xml_lexer.PCData t -> PCData (t, pos s)
	| Xml_lexer.Tag (tag, attr, true) -> Element (tag, pos s, attr, [])
	| Xml_lexer.Tag (tag, attr, false) -> Element (tag, pos s, attr, read_elems ~tag s)
	| t ->
		push t s;
		raise NoMoreData
and
