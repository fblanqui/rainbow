...
let children = function
	| Element (_,_,_,clist) -> clist
	| x -> raise (Not_element x)

(*let enum = function
	| Element (_,_,_,clist) -> List.to_enum clist
	| x -> raise (Not_element x)
*)

let iter f = function
	| Element (_,_,_,clist) -> List.iter f clist
	| x -> raise (Not_element x)

let map f = function
	| Element (_,_,_,clist) -> List.map f clist
	| x -> raise (Not_element x)

let fold f v = function
	| Element (_,_,_,clist) -> List.fold_left f v clist
	| x -> raise (Not_element x)
...
let to_string_fmt x =
	let rec loop ?(newl=false) tab = function
		...
		| Element (tag,_,alist,[PCData (text,_)]) ->
			Buffer.add_string tmp tab;
			Buffer.add_char tmp '<';
			Buffer.add_string tmp tag;
			List.iter buffer_attr alist;
