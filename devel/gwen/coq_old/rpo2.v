...
(* [lex] used in the definition of [o_size2]. *)
...
(* The proof of [lex_trans] used in the proof of [o_size2_trans]. *)
...
Proof.
  ...
  generalize (eq_bool_ok a1 a2); case (eq_bool a1 a2);
    [intro a1_eq_a2; subst a1 | intro a1_diff_a2].
  generalize (eq_bool_ok a2 a3); case (eq_bool a2 a3);
    [intro a2_eq_a3; subst a2 | intro a2_diff_a3].
  apply T2.
  intros _ a2_t_a3; assumption.
  generalize (eq_bool_ok a2 a3); case (eq_bool a2 a3);
    [intro a2_eq_a3; subst a2 | intro a2_diff_a3].
  generalize (eq_bool_ok a1 a3); case (eq_bool a1 a3);
    [intro a1_eq_a3; subst a1 | intro a1_diff_a3].
...
(* The proof of [wf_lex] used in the proof of [wf_size2].*)
...
Proof.
  ...
  intros [a1 a2]; revert a2; generalize (W1 a1);
    intros Wa1 a2; generalize (W2 a2).
  ...
  generalize (eq_bool_ok b1 a1); case (eq_bool b1 a1);
    [intro b1_eq_a1 | intro b1_diff_a1].
...
Record Precedence (A: Type) : Type := mkPrecedence {
  status : A -> status_type;
  prec : relation A;
  prec_eq: relation A;
  prec_bool : A -> A -> bool;
  prec_eq_bool : A -> A -> bool;
  prec_bool_ok : forall a1 a2,
    match prec_bool a1 a2 with true =>
      prec a1 a2 | false => ~prec a1 a2 end;
  prec_eq_bool_ok: forall a1 a2,
    match prec_eq_bool a1 a2 with true =>
      prec_eq a1 a2 | false => ~prec_eq a1 a2 end;
  prec_antisym : forall s, prec s s -> False;
  prec_transitive : transitive prec;
  prec_eq_transitive: transitive prec_eq;
...
Require Import cocc_term2 ASignature.
...
Section RPO.
  ...
  Variable status' : A -> status_type.

  Variable prec_nat' : A -> nat.

  Hypothesis prec_eq_status': forall f g,
    prec_eq' Sig prec_nat' f g -> status' f = status' g. 

  (* Define an instansiation of Record Precedence of type A. *)

  Definition build_Precedence : Precedence A := mkPrecedence status'
      (prec_bool' Sig prec_nat') (prec_eq_bool' Sig prec_nat')
      (prec_bool_ok' Sig prec_nat') (prec_eq_bool_ok' Sig prec_nat')
      (@prec_antisym' Sig prec_nat') (@prec_transitive' Sig prec_nat')
      (@prec_eq_transitive' Sig prec_nat') (@prec_eq_prec1' Sig prec_nat')
      (@prec_eq_prec2' Sig prec_nat') (@prec_eq_sym' Sig prec_nat')
      (@prec_eq_refl' Sig prec_nat')
      prec_eq_status' (@prec_not_prec_eq' Sig prec_nat').

(*  Notation rpo_term := (rpo_term Sig).*)
  Notation P := build_Precedence.

  (* REMARK: change to [rpo_term] to be able to distinguist between
     CoLor's term and rpo's term.*)

  Notation term := (term Sig).

  (* Define [equiv] type used in [rpo] type. *)

  (* REMARK: [equiv] has the same name in Coq's library
     [Relation_Definitions] and [Equivalence.v]. And [Eq] has a same
     name in [Datatypes.v] *)

  Inductive equiv : term -> term -> Prop :=
  | Eq : forall t, equiv t t
  | Eq_lex : 
    forall f g l1 l2, status P f = Lex -> status P g = Lex ->
      prec_eq P f g -> equiv_list_lex l1 l2 -> 
      equiv (Term f l1) (Term g l2) 
  | Eq_mul :
    forall f g l1 l2,  status P f = Mul -> status P g = Mul ->
  ...
  | Top_eq_lex : 
    forall f g l l', status P f = Lex -> status P g = Lex -> prec_eq P f g ->
      (length l = length l' \/ (length l' <= bb /\ length l <= bb)) -> rpo_lex bb l' l -> 
  ...
  Notation symbol := A (only parsing).
  ...
  unfold iter; simpl.
  case (prec_eq_bool' Sig prec_nat' f1 f2).
  ...
  Proof.
    ...
    case (prec_eq_bool' Sig prec_nat' f1 f2); [idtac | reflexivity].
  ...
  Proof.
    ...
    intros f l1 E_l1 t2 t3 t1_eq_t2 t2_eq_t3; 
      inversion t1_eq_t2 as
        [ | f' g' l1' l2 Sf Sg eq_f_g l1_eq_l2 H2 H'
          | f' g' l1' l2 Sf Sg eq_f_g P]; subst; trivial;
        inversion t2_eq_t3 as
          [ | f' g'' l2' l3 Sf' Sg' eq_f_g' l2_eq_l3 H2 H''
  ...
  Proof.
    ...
    assert (H2:= prec_eq_bool_ok). assert (H2':= H2 A RPO.P f1 f2).
    rewrite f1_eq_f2 in H2'. assert (H6:= prec_eq_status RPO.P f1 f2); trivial.
    assert (H7: status RPO.P f1 = status RPO.P f2). apply H6. trivial.
    rewrite <- H7. trivial. 
    assert (H2:= prec_eq_bool_ok). assert (H2':= H2 A RPO.P f1 f2).
    rewrite f1_eq_f2 in H2'. trivial.
    trivial. 
    ...
    assert (H3:= prec_eq_bool_ok). assert (H3':= H3 A P f2 f2).
    rewrite f1_diff_f2 in H3'. contradict H3'.
    apply prec_eq_refl. trivial.
    assert (H8:= prec_eq_bool_ok). assert (H8':= H8 A P f1 f2).
  ...
  Proof.
    ...
    inversion t_eq_t' as [ t'' | f g'' l1 l2 Statf Statg eq_f_g H |
      f g'' l1 l2 Statf Statg eq_f_g P]; subst.
  ...
  Proof.
    ...
    inversion t_eq_t' as [ t'' | g g' l1 l2 Stat Statg' g_eq_g' l1_eq_l2 |
      g g' l1 l2 Stat Statg' g_eq_g' P]; subst.
      (* 1/4 equivalence is syntactic identity *)
    trivial.
      (* 1/3 equivalence with Lex top symbol *)
  ...
  Lemma equiv_eval_is_sound_weak :
    ...
    intros _.
    case_eq (prec_eq_bool' Sig prec_nat' f1 f2); [intro f1_eq_f2 | intro f1_diff_f2].
    case_eq (status' f1). intros Status.
    intro H; apply Eq_lex; trivial.
    assert (H1:= prec_eq_status). assert (H1':= H1 A P f1 f2).
    assert (status P f1 = status P f2). apply H1'; trivial. trivial.
    assert (H3:= prec_eq_bool_ok'). assert (H3':= H3 Sig prec_nat' f1 f2).
...
(* 1/5 lexico_eval p s1 s2 l1 l2 = Some Greater_than *)
  ...
  Proof.
    ...
    assert (H1:= prec_eq_bool_ok). assert (H1':= H1 A P f1 f2).
    rewrite H in H1'. trivial.
    intro.
    case_eq (prec_bool P f1 f2).
    intros.
    assert (H1:= prec_bool_ok). assert (H1':= H1 A P f1 f2).
    rewrite H0 in H1'. trivial.
    intros.
    case_eq (prec_bool P f2 f1).
    intro prec'.
    assert (H1:= prec_bool_ok). assert (H1':= H1 A P f2 f1).
    rewrite prec' in H1'. trivial.
    intros.
    split.
    assert (H3:= prec_eq_bool_ok). assert (H3':= H3 A P f1 f2).
    rewrite H in H3'. trivial.
    split. 
    assert (H3:= prec_bool_ok). assert (H3':= H3 A P f1 f2).
    rewrite H0 in H3'. trivial.
    assert (H3:= prec_bool_ok). assert (H3':= H3 A P f2 f1).
  ...
  Proof.
    ...
    clear E1; 
      assert (H : forall v f l, var_in_term_list v l = true -> rpo rpo_infos.(bb)
        (Var Sig v) (Term f l)).
    intros v f l H; apply trans_clos_subterm_rpo; trivial.
    assert (H' : (In (Var Sig v) l \/ 
      (exists t, In t l /\ trans_clos (@direct_subterm Sig) (Var Sig v) t)) ->
    trans_clos (@direct_subterm Sig) (Var Sig v) (Term f l)).
    ...
    generalize (eq_var_bool_ok v v'); case (eq_var_bool v v');
      [intros v_eq_v' _ | intro v_diff_v'].
    ...
    apply (@Eq_lex f1 f2 l1 l2 Sf1). assert (H3:= prec_eq_status).
    assert (H3':= H3 A P f1 f2). assert (H4: status P f1 = status P f2).
    apply H3'; trivial. rewrite <- H4; trivial. trivial.
    ...
    intro L; apply (@Top_eq_lex rpo_infos.(bb) f1 f2 l2 l1 Sf1).
    assert (H3:= prec_eq_status).
    assert (H3':= H3 A P f1 f2).
    assert (H4: status P f1 = status P f2). apply H3';
    trivial. rewrite <- H4; trivial. trivial.
    left; apply sym_eq; apply beq_nat_true; assumption.
    destruct H' as [[H1 H2] | [[ll [t2 [l2' [ _ [H1 H2]]]]] |
      [ll [t1 [t2 [l1' [l2' [Hll [Ht [H' [H1 H2]]]]]]]]]]].
    destruct l2 as [ | a2 l2]; [apply False_rect; apply H2;
      apply refl_equal | subst l1; discriminate].
    subst l1 l2; rewrite length_app in L; do 2 rewrite length_map in L.
    apply False_rect; generalize (beq_nat_true _ _ L);
      clear L; induction ll as [ | [u1 u2] ll].
    discriminate.
    intro L; injection L; clear L; intro L; apply IHll; assumption.
    clear L H'; subst l1 l2; induction ll as [ | [u1 u2] ll].
    simpl; constructor 1; generalize (IHn t1 t2);
      rewrite Ht; intro; assumption.
    simpl; constructor 2.
    ...
    discriminate.
    intro L; injection L; clear L; intro L; apply IHll; assumption.
    intros u u_in_l1; rewrite mem_in_eq in u_in_l1;
      destruct u_in_l1 as [u' [u_eq_u' u'_in_l1]].
    ...
    right; rewrite (equiv_rpo_equiv_2 _ u_eq_u');
      generalize (IHn u' u2); rewrite u'_lt_u2; intro; assumption.
    rewrite (equiv_rpo_equiv_2 _ u_eq_u');
      generalize (IHn (Term f2 l2) u'); rewrite H''; intro; assumption.
    intros _; generalize (leb_complete (length l1) (rpo_infos.(bb)));
      case (leb (length l1) rpo_infos.(bb)); [idtac | simpl; trivial].
    generalize (leb_complete (length l2) (rpo_infos.(bb)));
      case (leb (length l2) (bb rpo_infos)); simpl; [idtac | trivial].
    intros L2 L1; apply (@Top_eq_lex rpo_infos.(bb) f1 f2 l2 l1 Sf1).
    assert (H3:= prec_eq_status). assert (H3':= H3 A P f1 f2).
    assert (H4: status P f1 = status P f2). apply H3'; trivial. rewrite <- H4; trivial. trivial.
    right; split; [apply L1 | apply L2]; apply refl_equal.
    clear L1 L2; destruct H' as [[H1 H2] |
      [[ll [t2 [l2' [Hll [H1 H2]]]]] | [ll [t1 [t2 [l1' [l2' [Hll [Ht [H' [H1 H2]]]]]]]]]]].
    ...
    assert (H3:= prec_eq_status). assert (H3':= H3 A P f1 f2).
    assert (H4: status P f1 = status P f2). apply H3'; trivial. rewrite <- H4; trivial. trivial.
    intro L; apply (@Top_eq_lex rpo_infos.(bb) f2 f1 l1 l2 Sf2).
    trivial. apply prec_eq_sym; trivial.
    left; apply beq_nat_true; assumption.
    destruct H' as [[H1 H2] | [[ll [t1 [l1' [ _ [H1 H2]]]]] |
      [ll [t1 [t2 [l1' [l2' [Hll [Ht [H' [H1 H2]]]]]]]]]]].
    destruct l1 as [ | a1 l1];
      [apply False_rect; apply H1; apply refl_equal | subst l2; discriminate].
    subst l1 l2; rewrite length_app in L; do 2 rewrite length_map in L.
    apply False_rect; generalize (beq_nat_true _ _ L);
      clear L; induction ll as [ | [u1 u2] ll].
    discriminate.
    ...
    simpl; constructor 1; generalize (IHn t1 t2); rewrite Ht; intro; assumption.
    simpl; constructor 2.
    apply (equiv_sym _ _ equiv_equiv); generalize (IHn u1 u2);
      rewrite (Hll _ _ (or_introl _ (refl_equal _))); intro; assumption.
    apply IHll; intros; apply Hll; right; assumption.
    destruct H' as [[H1 H2] | [[ll [t1 [l1' [_ [H1 H2]]]]]
      | [ll [t1 [t2 [l1' [l2' [Hll [Ht [H' [H1 H2]]]]]]]]]]].
    ...
    left; apply (equiv_trans _ _ equiv_equiv) with u'; trivial.
    apply (equiv_sym _ _ equiv_equiv); generalize (IHn u1 u');
      rewrite u'_eq_u1; intro; assumption.
    right; rewrite (equiv_rpo_equiv_2 _ u_eq_u'); generalize (IHn u1 u');
      rewrite u'_lt_u1; intro; assumption.
    rewrite (equiv_rpo_equiv_2 _ u_eq_u'); generalize (IHn (Term f1 l1) u');
    ...
    generalize (leb_complete (length l2) (rpo_infos.(bb)));
      case (leb (length l2) (bb rpo_infos)); simpl; [idtac | trivial].
    assert (H3:= prec_eq_status). assert (H3':= H3 A P f1 f2).
    assert (H4: status P f1 = status P f2). apply H3'; trivial.
    assert (Sf2: status P f2 = Lex). rewrite <- H4; trivial.
    intros L2 L1; apply (@Top_eq_lex rpo_infos.(bb) f2 f1 l1 l2 Sf2).
    trivial. apply prec_eq_sym;
    trivial.
    right; split; [apply L2 | apply L1]; apply refl_equal.
    clear L1 L2; destruct H' as [[H1 H2] | [[ll [t1 [l2' [Hll [H1 H2]]]]]
      | [ll [t1 [t2 [l1' [l2' [Hll [Ht [H' [H1 H2]]]]]]]]]]].
    subst l2; destruct l1 as [ | a1 l2];
      [apply False_rect; apply H1; apply refl_equal | constructor 3].
    subst l1 l2; induction ll as [ | [u1 u2] ll].
    simpl; constructor 3.
    simpl; constructor 2.
    apply (equiv_sym _ _ equiv_equiv); generalize (IHn u1 u2);
    ...
    apply (equiv_sym _ _ equiv_equiv); generalize (IHn u1 u2);
      rewrite (Hll _ _ (or_introl _ (refl_equal _))); intro; assumption.
    apply IHll; intros; apply Hll; right; assumption.
    clear L1 L2; destruct H' as [[H1 H2] | [[ll [t1 [l1' [Hll [H1 H2]]]]] |
      [ll [t1 [t2 [l1' [l2' [Hll [Ht [H' [H1 H2]]]]]]]]]]].
    intros; subst l2; contradiction.
    intros u u_in_l2; rewrite mem_in_eq in u_in_l2; destruct u_in_l2 as [u' [u_eq_u' u'_in_l2]].
    ...
    assert (H3:= prec_eq_status'). assert (H3':= H3 f1 f2).
    assert (H4: status' f1 = status' f2). apply H3'; trivial.
    assert (Sf2: status' f2 = Mul). rewrite <- H4; trivial. 
    rewrite <- H4 in Sf2.
