(* FIXME: No need to defined this file again. It is nothing but just
change the name. rpo_extension2 used in Coccinelle*)
...
Section S.
  ...
  Section MakePrecedence.
    ...
    Lemma prec_eq_bool_ok': forall a1 a2,
      match prec_eq_bool' a1 a2 with
        true => prec_eq' a1 a2
        | false => ~prec_eq' a1 a2
      end.
    ...
    Proof.
      intros a b c H1 H2;
        apply lt_trans with (prec_nat' b); assumption.
    Qed.
    
    Lemma prec_eq_transitive' : transitive prec_eq'.
      
    Proof.
      intros a b c H1 H2. unfold prec_eq'. unfold prec_eq' in H2.
      unfold prec_eq' in H1. assert (H3: prec_nat' a = prec_nat' b).
      apply eq_nat_eq.  trivial. assert (H4: prec_nat' b = prec_nat' c).
      apply eq_nat_eq. trivial. assert (H5: prec_nat' a = prec_nat' c). omega.
      rewrite H5. apply eq_nat_refl.
    Qed.

    Lemma prec_eq_prec1': forall a1 a2 a3, prec' a1 a2 -> prec_eq' a2 a3 -> prec' a1 a3.

    Proof.
      unfold prec'. unfold prec_eq'. intros. assert (prec_nat' a2 = prec_nat' a3).
      apply eq_nat_eq. trivial. rewrite <- H1. trivial. 
    Qed.

    Lemma prec_eq_prec2':  forall a1 a2 a3, prec' a1 a2 -> prec_eq' a1 a3 -> prec' a3 a2.

    Proof.
      unfold prec'. unfold prec_eq'. intros. assert (prec_nat' a1= prec_nat' a3).
      apply eq_nat_eq. trivial. rewrite <- H1. trivial. 
    Qed.
    ...
    Proof.
      unfold prec_eq'. intros. assert (prec_nat' s = prec_nat' t).
      apply eq_nat_eq. trivial. rewrite H0. apply eq_nat_refl.
    Qed.
  ...
  (* Use rpo2.Build_Precedence *)

    (*Definition build_Precedence := rpo2.Build_Precedence status
      prec_bool prec_eq_bool prec_bool_ok prec_eq_bool_ok prec_antisym prec_transitive
      prec_eq_transitive prec_eq_prec1 prec_eq_prec2 prec_eq_sym prec_eq_refl
      prec_eq_status prec_not_prec_eq. *)
...
Module MakeRpoExt (Eqt : equational_theory_spec.EqTh).
  ...
  Section S.
    ...
    Variable prec_eq_status: forall f, forall g,
      prec_eq prec f g -> status f = status g. 
...
Module MakeRpoSdpMarkedAFS (Eqt : equational_theory_spec.EqTh).
  ...
  Section S.
    ...
    Variable prec_eq_status: forall f, forall g,
      prec_eq prec_union f g -> status_union f = status_union g. 
