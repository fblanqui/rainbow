...
(*
   ...
   Re-define Module type into Section to be able to use in new
   formalization of Rainbow.
*)
...
Require Import term_spec EqUtil ASignature.

Section Make_Signature.
  ...
  Lemma eq_bool_symb_ok : forall a1 a2,
    match eq_bool_symb a1 a2 with true => a1 = a2 | false => ~ a1 = a2 end.
  ...
  Definition arity' (f: Sig) : arity_type  := Free (ASignature.arity f).
...
Section Var.
  ...
  Definition var_A := nat.

  Definition var_eq_bool := beq_nat.

  Lemma var_eq_bool_ok: forall a1 a2,
    match var_eq_bool a1 a2 with true => a1 = a2 | false => ~ a1 = a2 end.
...
Section Make'.
  ...
  (* Define symbol of type [symbol Sig] for Term in CoLoR has [symbol
  Sig] *)
            ...
            let eq_bool_list :=
              (fix eq_bool_list (l1 l2: list term) {struct l1} : bool :=
                match l1, l2 with
                  ...
                  | nil, (_ :: _) => false
                  | (_::_), nil => false
                  | t1 :: k1, t2 :: k2 => if t_eq_bool t1 t2 then eq_bool_list k1 k2 else false
                end) in
              eq_bool_list l1 l2
          else false
    end.
  ...
  Lemma t_eq_bool_ok : forall t1 t2,
    match t_eq_bool t1 t2 with true => t1 = t2 | false => t1<> t2 end.
