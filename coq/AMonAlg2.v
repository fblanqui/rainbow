...
- Frédéric Blanqui, 2014-02-25 (classes)
...
Section MonotoneAlgebraResults.
  ...
  Notation ma_succ   := (ma_succ Sig).
  Notation ma_succeq := (ma_succeq Sig).
  Notation IR_succ   := (IR I ma_succ).
  Notation IR_succeq := (IR I ma_succeq).

  Lemma absorb_succ_succeq : absorbs_left IR_succ IR_succeq.

  Proof.
    intros x z xz val.
    apply ma_succ_succeq_compat.
  ...
  Proof.
    ...
    exact (ma_monotone_succeq Sig).
  Qed.

  Definition part_succeq := rule_partition (ma_succeq'_dec Sig).
  Definition part_succ   := rule_partition (ma_succ'_dec Sig).
  ...
  Proof.
    intros R l r lr.
    apply ma_succ'_sub.
    apply partition_by_rel_true with term (ma_succ'_dec Sig).
...
(** relative termination criterion with monotone algebras *)
  ...
  Section RelativeTermination.
    ...
    Lemma ma_compat_red_mod : 
      monotone I ma_succ -> compat IR_succ R ->
                            compat IR_succeq E -> WF (red_mod E R).
    ...
    Proof.
      ...
      apply IR_WF. exact (ma_succ_wf Sig).
    Qed.
...
(** top termination (for DP setting) criterion with monotone algebras *)
  ...
  Section RelativeTopTermination.
    ...
    Lemma ma_compat_hd_red_mod : 
      compat IR_succ R -> compat IR_succeq E -> WF (hd_red_mod E R).
...
(** results and tactics for proving relative termination 
  ...
  Section RelativeTerminationCriterion.
    ...
    Proof.
      ...
      apply ma_succeq'_sub.
      apply partition_by_rel_true with term (ma_succeq'_dec Sig).
...
(** results and tactics for proving relative top termination 
  ...
  Section RelativeTopTerminationCriterion.
    ...
    Proof.
      ...
      apply ma_succeq'_sub. apply partition_by_rel_true with term (ma_succeq'_dec Sig).
      apply rule_partition_left with R. hyp.
    Qed.
